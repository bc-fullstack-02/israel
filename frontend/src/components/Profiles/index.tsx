...
import { getAuthHeader } from "../../services/auto"
...
function Profiles() {
    ...
    return (
        <div className="basis-5/6">
            <Heading className="border-b border-orange-400 mt-4">
                <Text size='lg' className="font-extrabold ml-5">Amigos</Text>
                <div className="flex flex-row items-center ml-5 my-4">
                    <UserCircle size={48} weight='light' className="text-green-400" />
                    <Text className="font-extrabold ml-2">{user}</Text>
                </div>
            </Heading>
            <ul>
                {profiles.map((profile) => (
                    <li key={profile._id} className="border-b border-orange-400 mt-4 pl-5">
